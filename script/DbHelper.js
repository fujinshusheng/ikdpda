var db = {
    ...
    submit: function (Cmd, data, fun, _IP1, option) {
        ...
        var linshiUrl = db.config.url;
        //接口组合
        var urls = "http://" + db.Data.get("IP") + "/" + linshiUrl + '?cmd=' + Cmd + '&cmdname=' + Cmd + '&sb=手持PDA';
        if (_IP1 != undefined && _IP1 != "") urls = "http://" + _IP1 + "/" + linshiUrl + '?cmd=' + Cmd + '&cmdname=' + Cmd + '&sb=手持PDA';
		
        //layer.msg("当前请求网址：" + urls);
...
;
var jw = {};
jw.scan_trim = function () {
    //$("#scanDom").val('');
    $("#scanDom").attr("readonly", "readonly");
    setTimeout(function () {
        $("#scanDom").focus();
        $("#scanDom").removeAttr("readonly")
    }, 100)
};
//停止自动焦点
jw.stopsetInterval = false;
...
$(function () {
    window.setTimeout(function () {
        ...
        // //新采购的TC20扫描枪
        // if (deviceName == 'Zebra Technologies') {
            //alert(deviceName);
            //区域编号
            ...
            if (jwquyuID != null) {
                layer.msg('扫描程序加载成功');
                if ($('#scanDom').length == 0) {
                    $("body").append('<textarea id="scanDom" style="position:absolute;left:0;top:-500px;" autocomplete="off"></textarea>');
                    $("#scanDom").focus();
                }
                $(document).on('keydown', '#scanDom', function (e) {
                    var e = e || w.event; e.stopPropagation();
                    if (e.keyCode == 13) {
                        var a = $('#scanDom').val().trim();
                        $("#scanDom").val('');
                        //layer.msg('扫描内容：' + a);
                        try {
                            a && jw.scanfun && jw.scanfun(a)
                        } catch (e) { } finally {
                            jw.scan_trim()
                        }
                    }
                    ...
                    if ($(e.target).closest(':input').length) {
                        var e = e || w.event;
                        e.stopPropagation()
                        var _inputtype = $(e.target).attr("type");
                        var ispass = false;
                        if (_inputtype != 'submite' && _inputtype != 'button') {
                            if ($(e.target).attr("readonly") == undefined)
                                ispass = true;
                        }
                        if (ispass) jw.stopsetInterval = true;
                    } else {
                        jw.stopsetInterval = false;
                        jw.scan_trim()
                    }
                ...
                window.setInterval(function () {
                    if (!jw.stopsetInterval) jw.scan_trim();
                }, 1500);
            }
        //}
    }, 700);
